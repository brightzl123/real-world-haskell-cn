...
[@sancao2:我觉得作者不是不知道类型类应该与接口和多态方法类比，他这么说的原因是下面他自己的注释"When is a class not a class?"里面说的，因为类型类的关键词是class，传统面向对象编程里面的关键词也是class，囧rz。]
...
[@sancao2:这句话为什么原文加引号，其实是帮助读者解读编译器输出"``(BasicEq a) => a -> a -> Bool``" 。]
让我们快速地浏览一遍为某个特定类型定义的 ``isEqual`` 吧。
[@sancao2:注意(定义实例的手法)关键词变成了 ``instance``，接着 ``a`` 被替换成 ``Bool`` 类型，然后才是类型类须实现的函数的定义。]
...
[@sancao2:作者给我们解读编译器"``No instance for (BasicEq [Char])``"的报警/抱怨。
学会看编译器的输出是非常重要的，真是用心良苦。]
并且其建议（"``Possible fix``"）到我们可以通过定义基于 ``[Char]`` 实例类型的 ``BasicEq`` 。
[@sancao2:编译器"``Possible fix: add an instance declaration for (BasicEq [Char])``"的解读。]
...
[@sancao2:因为 ``Color`` 的类型定义在naiveeq.hs中，所以我就把关于 ``Color`` 的类型类实例都放这里了，下同。]
...
[@sancao2:就是下面编译器提示的:"Read a"， 代表 ``Read`` 实例类型。]
::
    Prelude> :type read
...
[@sancao2:``(read "Red")::Color`` 输出 ``Color 1: Red`` 的结果而不是 ``Red`` ，是因为前面重载了 ``show`` 函数的结果。]
注意到最后的尝试产生了错误。
那是因为我们的编译器没有聪明到可以处理置位(leading，包括前置和后置)的空格。
你可以改进他，通过些改你的 ``Read`` 实例以忽略任何置位的空格。
这在Haskell程序中是常见的做法。

[@sancao2:因为某个原因前面报错了，所以搜索了一下，不小心找到了。
...
[@sancao2:原作者的例子实在不如huangz等译者的清爽，不翻译，将实验贴在这里。
::
...
[@sancao2:6.3 表非常重要，能帮助我们看懂编译器的对函数的报错。
...
[@sancao2:这句话原文是 ``Eq`` class，我想作者已经不区分 ``typeclass`` 和 ``class`` 了。
...
[@sancao2，若没有 ``{-# LANGUAGE FlexibleInstances #-}`` 这条编译选项，就会产生下面的结果。
其实编译器的 ``fix`` 提示给大家了。
::
...
[@sancao2:不得不提一句，其实这种开发方法，就是面向接口的。（台湾人叫做把Oriented，译成xx导向，更动态的。大陆译成面向xx。）
先定义接口，然后再去实现。
...
[@sancao2:上面的作者注释，其实说的是库的 ``OverlappingInstances`` 不会污染用户的代码。
或者说，其不用传导给用户代码的编译选项。
...
[@sancao2:上面那句 ``[Char]`` 原文没有 ``[]`` ，应该是错了。]

作为结果，如果有人应用(apply) ``show`` 于 ``[Char]`` 数值， ``showList`` 的实现会被选上，并且将会正确地渲染字符串，通过使用括号们。

[@sancao2:我很想说，这里的apply的作者的用词，其实相当于调用库函数里面apply方法， ``apply show [Char]`` 。
...
[@sancao2:lateral thinking，就是这里的说的在"覆盖同例"这个扩展出现之前的时间里（“Haskell 98”之前没有），前人是怎么hack这个问题的。
然后hack前人的方法的这种思维。
...
::
    ...
    *Main> case (D undefined) of D _ -> 1
    1
[@sancao2:做这个实验要先加载“Newtype.hs"，其中定义了 ``Ｄ`` 。]
...
还有我的输出有23种了， ``...plus 23 others`` 。]
::
   ghci> :load Monomorphism            ----旧版本ghc的报错。
   [1 of 1] Compiling Main             ( Monomorphism.hs, interpreted )
...
[@sancao2:下面那段话，引用原版错误输出，所以我把他也贴出来。]
